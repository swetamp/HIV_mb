...
ggplot(sampleDF, aes(fill=hiv, x=cluster, y=hiv)) + geom_bar(stat = "identity", position = "fill", width = 0.9) + 
  scale_fill_manual(values = c('#800000FF','#155F83FF', '#FFA319FF'))+
  xlab('Cluster')+
  theme_classic()
#doesn't work; need to create frequency of HIV category by cluster, turn them into frequencies, and then create a stacked barplot from that like we did with the PCV13 paper

#UPDATE to the above code: saves time because you don't have to manually create frequencies; use stat = "count" and get rid of your y= in aes
ggplot(sampleDF, aes(fill=hiv2, x=cluster)) + geom_bar(stat = "count", position = "fill", width = 0.9) + 
  scale_fill_manual(values = c('#800000FF','#155F83FF', '#FFA319FF'))+
  xlab('Cluster')+
  theme_classic()
...
#generate frequency data and calculate the proportions of HIV category for each cluster separately; then rbind back together
freq <- table(sampleDF$hiv2, sampleDF$cluster)
freq <- as.data.frame(freq)
colnames(freq) <- c("hiv", "cluster", "freq")

c1 <- subset(freq, cluster==1)
c1$newfreq <- (c1$freq)/44
sum(c1$newfreq)

c2 <- subset(freq, cluster==2)
c2$newfreq <- (c2$freq)/27
sum(c2$newfreq)

c3 <- subset(freq, cluster==3)
c3$newfreq <- (c3$freq)/16
sum(c3$newfreq)

c4 <- subset(freq, cluster==4)
c4$newfreq <- (c4$freq)/29
sum(c4$newfreq)

c5 <- subset(freq, cluster==5)
c5$newfreq <- (c5$freq)/27
sum(c5$newfreq)

#extra check
44+27+16+29+27 #all denominators add up to 143

freq <- rbind(c1, c2, c3, c4, c5)

#Now create figure
hiv_clust <- ggplot(freq, aes(fill=hiv, x=cluster, y=newfreq)) + geom_bar(stat= "identity", position = "fill", width = 0.9) +
  scale_fill_manual(values = c('#800000FF','#155F83FF', '#FFA319FF')) +
...
#For coding learning; the above took way too much effort. Can we get the same results with geom_bar stat = count?
...
To answer this, we will subset CLWH from our chi.spec pseq object and repeat the clustering process. 

```{r}
hiv.spec = subset_samples(chi.spec, hiv == "Infected")

DistType = t(as.data.frame(vegan_otu(hiv.spec)))

data.dist = vegdist(vegan_otu(hiv.spec), method = "bray")
...
#8 seems to be the ideal number for CLWH (though the CH index values are notably lower than for our overall dataset)
```

```{r}
#Set new number of clusters = to k based on above plot; we'll use 8
...
barplot(obs.pcoa$eig)
(kip <- 100 * obs.pcoa$eig/sum(obs.pcoa$eig))
cumsum(kip) #viewing the data in a 3d space extracts 48% of the info; we'd have to view in 19 axes to see 99% of the contained information. On the other hand, Alex's PC% were relatively small and the whole point of these plots is dimension reduction. Can d/w MK; do we need to re-do with a higher number of axes?

s.class(obs.pcoa$li, fac=as.factor(data.cluster), grid=T,  col = c(1:8))
  #gives you a cool figure!
...
chisp2 <- c("#293352", "#800000FF", "#767676FF", "#FFA319FF", "#8A9045FF", "#155F83FF", "#C16622FF", "#4D004B", "#8F3931FF", "#58593FFF", "#350E20FF")
show_col(chisp2)
#made chisp3 and chisp4 above, both with 5 colors, so here we will make chisp5 with 8 colors
chisp5 <- c("#293352", "#800000FF", "#767676FF", "#FFA319FF", "#8A9045FF", "#155F83FF", "#C16622FF", "#4D004B")
show_col(chisp5)

library(ggpubr)
clust_clwh <- ggplot(data = p, aes(x = A1, y = A2)) +
  geom_point(data = p, aes(color = data.cluster), alpha=1, size=1.0) + 
  scale_color_manual(values = chisp5) +
  theme_classic() +
    theme(
    legend.text = element_text(size = 10),
    legend.title = element_text(size = 12),
...
clust_clwh
#even though 8 had the highest C-H index, one of the clusters (#7) has 1 specimen and one (#8) has 3, so not able to draw ellipses around them. Seems not ideal; we can save this and then repeat with 6 clusters; the second highest C-H index is >8, but with only 44 kids this is also not going to be ideal, so will go with the a number <8 with the second highest C-H index (and 6 and 7 look equivalent on graph above)

#Question: how to tell PC %? Standard axes are A1 and A2 instead of PC% and PC% --> created a pcoa plot by cluster below and used those PC%; make sure this is ok

#save for ease of review; once you figure out how to update the axes accurately and finalize the figure, will need to save as an eps file (sample code below)
png(file="/Users/swetapatel/OneDrive - Duke University/Fogarty coding/clwh_clust8.png",
    width = 12, height = 9, units = 'in', res = 600)
clust_clwh
dev.off()
...
###NOW WITH 6 CLUSTERS###
#Set new number of clusters = to k based on above plot; we'll use 6
...
s.class(obs.pcoa$li, fac=as.factor(data.cluster), grid=T,  col = c(1:6))
  #gives you a cool figure!
...
chisp2 <- c("#293352", "#800000FF", "#767676FF", "#FFA319FF", "#8A9045FF", "#155F83FF", "#C16622FF", "#4D004B", "#8F3931FF", "#58593FFF", "#350E20FF")
show_col(chisp2)
#made chisp3 and chisp4 with 5 colors and chisp5 with 8 colors, so now we'll do chisp6 with 6 colors
chisp6 <- c("#155F83FF", "#8F3931FF", "#FFA319FF", "#8A9045FF", "#C16622FF", "#767676FF")
show_col(chisp6)
...
clust_clwh
#now have 1 group where ellipse not possible (group 6); not sure how good this clustering is, but we'll proceed with further analyses

#save for ease of review; once you figure out how to update the axes accurately and finalize the figure, will need to save as an eps file (sample code below)
png(file="/Users/swetapatel/OneDrive - Duke University/Fogarty coding/clwh_clust6.png",
    width = 12, height = 9, units = 'in', res = 600)
clust_clwh
dev.off()
```

- next, we will add the cluster info as a variable into our metadata, and generate our relevant HIV-specific variables

```{r}
#Add cluster to metadata file as a variable
sample_data(hiv.spec)$cluster = as.factor(data.cluster)

sampleDF = sample_data(hiv.spec) %>%
  data.frame(.)

#Now create variables for viral load and CD4
chi_hiv <- sampleDF #so we can use our original code for variable creation
#I want the most recent viral load values for my analysis. Will try creating a new variable to ID which value is more recent first:
#missing 3 vl1 and 14vl2's
chi_hiv$newest_vL <- NA
chi_hiv$newest_vL [chi_hiv$child_vl1_weeks < chi_hiv$child_vl2_weeks] <- 1
...
#save chi_cd4 with 6 clusters as a csv file in case we need to return to it
write.csv(chi_cd4, '/Users/swetapatel/OneDrive - Duke University/Fogarty coding/metadata_clust_hiv6.csv')
```
...
#do our clusters associate with any of our demographic variables?
#age by cluster
a <- ggplot(sampleDF, aes(x=cluster, y=age)) +
  geom_boxplot(fill=chisp6) +
  xlab('Cluster') + 
  ylab('Age (Months)') +
  theme_classic()
...
sampleDF$tmpsmx <- NA
sampleDF$tmpsmx [sampleDF$child_tmpsmx == 1] <- "Yes"
sampleDF$tmpsmx[sampleDF$child_tmpsmx == 2] <- "No"
table(sampleDF$tmpsmx)

d <- ggplot(sampleDF, aes(fill=tmpsmx, x=cluster)) + geom_bar(stat = "count", position = "fill", width = 0.9) + 
  scale_fill_manual(values = c('#800000FF','#155F83FF'))+
  xlab('Cluster')+
  theme_classic()

#abx use by cluster
e <- ggplot(sampleDF, aes(fill=abx, x=cluster)) + geom_bar(stat = "count", position = "fill", width = 0.9) +
  scale_fill_manual(values = c('#800000FF','#155F83FF'))+
  xlab('Cluster')+
  theme_classic()
...
png(file="/Users/swetapatel/OneDrive - Duke University/Fogarty coding/clwh_clust6bar.png",
    width = 12, height = 9, units = 'in', res = 600)
clust_hivcomb
dev.off()

#Visual trends: cluster 3 has the highest number of viremic and immunosuppressed children. Cluster 1 only has virally suppressed children, and cluster 5 only has immunocompetent children. We will check comparisons of 1 vs others, 3 vs others, and 5 vs others. Can also check 2-3 vs others for cd4.
...
chi_cd4$clust23[chi_cd4$cluster == "1" | chi_cd4$cluster == "4" | chi_cd4$cluster == "5" | chi_cd4$cluster == "6"] <- 0
...
#Age
tapply(sampleDF$age, sampleDF$cluster, summary)
kruskal.test(sampleDF$age, sampleDF$cluster) #p<0.001

#Sex
table(sampleDF$sex, sampleDF$cluster)
prop.table(table(sampleDF$sex, sampleDF$cluster), 2)
fisher.test(sampleDF$sex, sampleDF$cluster)

#TMP-SMX use
table(sampleDF$tmpsmx, sampleDF$cluster)
prop.table(table(sampleDF$tmpsmx, sampleDF$cluster), 2)
fisher.test(sampleDF$tmpsmx, sampleDF$cluster)
...
summary(table(chi_viral$vl_suppr, chi_viral$cluster))
...
What were the microbiome characteristics of the 6 clusters generated in our sub-analysis of CLWH?
```{r}
Shannon = estimate_richness(hiv.spec, measures = "Shannon")%>%
  cbind(., sampleDF, by = "names") #used Alex's code but this gives you a new dataframe to use called "Shannon"

tapply(Shannon$Shannon, Shannon$cluster, summary)
kruskal.test(Shannon$Shannon, Shannon$cluster) #P=0.06

shan_hiv <- ggplot(Shannon, aes(x=cluster, y=Shannon, fill=cluster)) + 
  geom_boxplot() + labs(x="Microbiome profile (CLWH only)", y = "Shannon index")+
  scale_fill_manual(values=chisp6) +
  theme_classic() +
  theme(
    legend.text = element_text(size = 10),
    legend.position = "none",
...
#save for ease of review; if you include in manuscript, will need to save as an eps file (sample code below)
png(file="/Users/swetapatel/OneDrive - Duke University/Fogarty coding/SDI_by_clust_hiv.png",
    width = 12, height = 9, units = 'in', res = 600)
shan_hiv
dev.off()

# #save as .eps file to comply with Lancet Microbe requirements
# ggsave(file="/Users/swetapatel/OneDrive - Duke University/Fogarty coding/SDI_by_clust_hiv.eps", width = 7, height = 3.5, units = "in")
# shan
# dev.off()
```
...
psC1 = subset_samples(hiv.spec, sample_data(hiv.spec)$cluster == 1)
speciesDF = as.data.frame(cbind((otu_table(psC1, taxa_are_rows = FALSE)), tax_table(psC1)))
speciesDF$Species <- gsub(";", NA, speciesDF$Species)
speciesDF <- speciesDF %>% drop_na(Species) #Go from 6709 --> 6162 rows
row.names(speciesDF) <-  speciesDF$Species 
...
#now repeat this process for clusters 2-6, saving AvgDF2-6 in the process so we can bind them all together at the end

psC1 = subset_samples(hiv.spec, sample_data(hiv.spec)$cluster == 2)
speciesDF = as.data.frame(cbind((otu_table(psC1, taxa_are_rows = FALSE)), tax_table(psC1)))
speciesDF$Species <- gsub(";", NA, speciesDF$Species)
speciesDF <- speciesDF %>% drop_na(Species) #Go from 6709 --> 6162 rows
row.names(speciesDF) <-  speciesDF$Species 
...
psC1 = subset_samples(hiv.spec, sample_data(hiv.spec)$cluster == 3)
speciesDF = as.data.frame(cbind((otu_table(psC1, taxa_are_rows = FALSE)), tax_table(psC1)))
speciesDF$Species <- gsub(";", NA, speciesDF$Species)
speciesDF <- speciesDF %>% drop_na(Species) #Go from 6709 --> 6162 rows
row.names(speciesDF) <-  speciesDF$Species 
...
psC1 = subset_samples(hiv.spec, sample_data(hiv.spec)$cluster == 4)
speciesDF = as.data.frame(cbind((otu_table(psC1, taxa_are_rows = FALSE)), tax_table(psC1)))
speciesDF$Species <- gsub(";", NA, speciesDF$Species)
speciesDF <- speciesDF %>% drop_na(Species) #Go from 6709 --> 6162 rows
row.names(speciesDF) <-  speciesDF$Species 
...
psC1 = subset_samples(hiv.spec, sample_data(hiv.spec)$cluster == 5)
speciesDF = as.data.frame(cbind((otu_table(psC1, taxa_are_rows = FALSE)), tax_table(psC1)))
speciesDF$Species <- gsub(";", NA, speciesDF$Species)
speciesDF <- speciesDF %>% drop_na(Species) #Go from 6709 --> 6162 rows
row.names(speciesDF) <-  speciesDF$Species 
...
psC1 = subset_samples(hiv.spec, sample_data(hiv.spec)$cluster == 6)
speciesDF = as.data.frame(cbind((otu_table(psC1, taxa_are_rows = FALSE)), tax_table(psC1)))
speciesDF$Species <- gsub(";", NA, speciesDF$Species)
speciesDF <- speciesDF %>% drop_na(Species) #Go from 6709 --> 6162 rows
row.names(speciesDF) <-  speciesDF$Species 
...
#Now combine all your row means and pull out the 10 most common species for consistency with our other species barplots
AvgDF = cbind(AvgDF1, AvgDF2, AvgDF3, AvgDF4, AvgDF5, AvgDF6) %>%
  data.frame(.)
rownames(AvgDF) = rownames(speciesDF)
AvgDF$Avg = rowMeans(AvgDF)
...
f$tax = factor(f$tax, levels=c("Corynebacterium propinquum", "Corynebacterium pseudodiphtheriticum", "Dolosigranulum pigrum","Haemophilus influenzae","Micrococcus luteus", "Moraxella catarrhalis", "Moraxella lincolnii", "Moraxella nonliquefaciens", "Staphylococcus aureus","Streptococcus pneumoniae","Other"))

#rename tax so it looks nicer in the table legend
colnames(f)[colnames(f) == "tax"] <- "Species"

subject_plot4 <- ggplot(f[order(f$Species, decreasing = TRUE),], aes(x=variable, y=value, fill=Species)) + 
  geom_bar(stat="identity", position="fill") + theme_classic() + 
  theme(legend.position="right", 
  legend.text=element_text(size=10, face="italic"),
  axis.title.y = element_text(size=12, margin=margin(0,20,0,0)), axis.text.y = element_text(size=10), 
...
#save for ease of review; if you include in manuscript, will need to save as an eps file (sample code below)
png(file="/Users/swetapatel/OneDrive - Duke University/Fogarty coding/cluster_rel_abund_hiv.png",
    width = 12, height = 9, units = 'in', res = 600)
subject_plot4
dev.off()

#cluster 1: 64% Coryne + D.pig; cluster 5 = 69% Coryne + D.pig

#NOW: use cowplot to combine our PCoA plot and composition bar plot into one figure
fig6 <- plot_grid(clust_clwh, subject_plot4, labels = c('A', 'B'), align = "h", axis = "b", label_size = 12, rel_widths = c(1, 1.5))
fig6

#save as .eps file to comply with Lancet Microbe requirements
ggsave(file="/Users/swetapatel/OneDrive - Duke University/Fogarty coding/fig6_clust_hiv.eps", width = 11, height = 3.5, units = "in")
fig6
dev.off()
```
...
For consistency, we will use the same filtering criteria that we used for our Maaslin2 analyses and filter taxa with fewer than 50 counts across the dataset.
