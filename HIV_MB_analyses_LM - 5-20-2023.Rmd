...
#MULTIVARIABLE MODEL
#Need to set reference for HIV variable and other categorical variables
...
#wood
div_child$wood <- as.factor(div_child$wood)
div_child$wood <- relevel(div_child$wood, ref = "0")
#season
div_child$season <- as.factor(div_child$season)
div_child$season <- relevel(div_child$season, ref = "Dry")
...
#LM editor comments 4/2024: asked if output of linear regression models available so we will add. ideally can present estimate, 95% CI, and p value but will confirm with MK.
confint(div_mv)
```
...
p1a <- plot_ordination(chi_clr, clr_pcoa, color = "hiv2") +
...
###UPDATE JUNE 2025: JID Reviewer asked for p values on the figure so we are regenerating each panel so we can add the P values to our PCOA plots
#will use annotate package in R; source: https://ggplot2.tidyverse.org/reference/annotate.html
#will match our font size to the axis TEXT font size (10); how to do this: https://stackoverflow.com/questions/65076492/ggplot-size-of-annotate-vs-size-of-element-text

p1 <- p1a + annotate("text", x = 35, y = -30, label = "p = 0.043", size = 10/.pt)
...
#### MaAsLin2 barplot
**UPDATE 4/2024: need to add labels to x-axes; as we had to add the enrollment figure as fig 1, pcoa/rel abund plots as fig 2, unsupervised clustering as fig 3, our maaslin fig is now fig 4**
- using huttenhower tutorial, confirmed that our x axis label should be "effect size" (https://huttenhower.sph.harvard.edu/maaslin/)
...
#UPDATE JULY 2025: JID does not accept pdf files, so will save as eps --> apparently latest OS does not support opening eps files, so will save as a TIFF
#ggsave("/Users/swetapatel/OneDrive - Duke University/Fogarty coding/Fig4NEW(maaslin)_May2024.pdf", plot = fig3, dpi = 300, width = 12, height = 5)

#ggsave(file="/Users/swetapatel/OneDrive - Duke University/Fogarty coding/Fig4NEW(maaslin)_July2025.eps", width = 12, height = 5, units = "in")
# fig3
# dev.off()

ggsave(file="/Users/swetapatel/OneDrive - Duke University/Fogarty coding/Fig4NEW(maaslin)_July2025.tiff", width = 12, height = 5, units = "in", device='tiff', dpi=700)
fig3
dev.off()
```
...
###UPDATE 12-10-23: MK wants names for the profiles, not numbers
#Create new var in P that corresponds to data.cluster for profile names
class(p$data.cluster) #factor
p$dcname <- NA
p$dcname [p$data.cluster == "1"] <- "CPD"
p$dcname [p$data.cluster == "2"] <- "BIO"
p$dcname [p$data.cluster == "3"] <- "MCA"
p$dcname [p$data.cluster == "4"] <- "MNO"
p$dcname [p$data.cluster == "5"] <- "CPR"
p$dcname <- as.factor(p$dcname)
table(p$data.cluster, p$dcname)
p$dcname <- reorder(p$dcname, new.order = c("CPD", "BIO", "MCA", "MNO", "CPR"))

library(ggpubr)
clust2 <- ggplot(data = p, aes(x = A1, y = A2)) +
  geom_point(data = p, aes(color = dcname), alpha=1, size=1.0) + 
  scale_color_manual(values = chisp3) +
  theme_classic() +
    theme(
    legend.text = element_text(size = 10),
    ...
    axis.text = element_text(size = 8),
    axis.title = element_text(size = 10))+
  stat_ellipse(aes(color = dcname)) +
  stat_stars(aes(color = dcname), show.legend = T) +
  xlab("PC1 (22.6%)") +
  ylab("PC2 (17.3%)") +
  labs(color = "Microbiome Profile")+ 
  scale_y_continuous(labels = scales::number_format(accuracy = 0.1))
...
fig5 <- plot_grid(clust2, clust_plot, labels = c('A', 'B'), align = "h", axis = "b", label_size = 12, rel_widths = c(1, 1.5))
fig5
...
#UPDATED 12-10-23 with new profile names per MK
ggsave("/Users/swetapatel/OneDrive - Duke University/Fogarty coding/Fig3NEW(clust)_V3.pdf", plot = fig5, dpi = 300, width = 11, height = 3.5)
...
###UPDATE 12-10-23; REDOING Cluster figure as above so need to rename the numeric labels and use abbreviations instead
fig <- read.csv('/Users/swetapatel/OneDrive - Duke University/Fogarty coding/cluster_comp.csv')
#redo our column names
colnames(fig)[colnames(fig) == "tax"] <- "Species"
colnames(fig)[colnames(fig) == "X1"] <- "CPD"
colnames(fig)[colnames(fig) == "X2"] <- "BIO"
colnames(fig)[colnames(fig) == "X3"] <- "MCA"
colnames(fig)[colnames(fig) == "X4"] <- "MLI"
colnames(fig)[colnames(fig) == "X5"] <- "CPR"

fig$Species = factor(fig$Species, levels=c("Corynebacterium accolens", "Corynebacterium propinquum", "Corynebacterium pseudodiphtheriticum", "Dolosigranulum pigrum","Haemophilus influenzae","Moraxella catarrhalis", "Moraxella lincolnii", "Moraxella nonliquefaciens", "Staphylococcus aureus","Streptococcus pneumoniae","Other"))

clust_plot2 <- ggplot(fig[order(fig$Species, decreasing = TRUE),], aes(x=variable, y=value, fill=Species)) + 
  geom_bar(stat="identity", position="fill") + theme_classic() + 
  theme(legend.position="right", 
  legend.text=element_markdown(),
  legend.title = element_text(size=10),
...
#UPDATE 5-20-2024: From LM Editor: would like us to add the correlation value for the darkest orange in our legend. We submitted an annotated version of Fig4SparCC_V2 with our revised manuscript, so will re-run the entire SpiecEasi set of code (not copied, but re-ran code from line 2523-) and re-do the figure (code copied below) with legend modified. Then we will check it against our Fig4SparCC_V2 as the ONLY difference between the 2 figures should be the legend. 
SparccPcsv <- read.csv("/Users/swetapatel/OneDrive - Duke University/Fogarty coding/SparCCredo_cortax2_test.csv")

fdrThresh2 <- 0.1 # fdr threshold
sparccOkP2 <- SparccPcsv%>% filter(fdr < fdrThresh2) 

SparccP_plot2f <- SparccPcsv %>% ggplot(aes(x = Var2, y = Var1, fill = cor)) + geom_tile() +
  scale_fill_gradientn(
  colors = c("#800000FF", "white", "#4D004B"), limits = c(-1, 1)) + scale_x_discrete(limits=rev, position = "top") + scale_y_discrete(limits=rev, position = "right") + theme(
  axis.text.x = element_markdown(angle = 45, hjust = 0, face = "italic"),
  axis.text.y = element_markdown(face = "italic"),
  legend.title = element_text(size = 9),
  legend.text.align = 1,
...
ggsave("/Users/swetapatel/OneDrive - Duke University/Fogarty coding/Fig4SparCC_June24.pdf", plot = SparccP_plot2f, dpi = 300, width = 10, height = 7)

#UPDATE 6-19-2024: will try running sparcc again without the cormat function to see if we can generate a heatmap that covers the whole grid, not just half

#UPDATE 6-25-2024: the output of sparcc should not change from run to run if we are setting our seed; but we may need to set the seed right before we run the sparcc and pvalue bootstrapping commands, so we will do that and generate the dataframe we use to create our heat map twice, so we can ensure reproducibility
#This worked! The SparccP dataframe generated at the end yielded the same results when we set the seed before the sparcc command and again before bootstrapping the p values
#NEXT: re-run our ORIGINAL code to generate half the heatmap, make sure the values correlate, and then export this to Cytoscape to regenerate our co-occurrence diagram. UPDATE: we needed to add another set.seed function before the fdr variable was created, so both versions of sparcc regenerated
...
#Plotting SparCC correlation
plotableSparcc2 <- sparcc.chi1$Cor %>% reshape2::melt() %>% na.omit()
Sparcc_plot2 <- plotableSparcc2 %>% ggplot(aes(x = Var2, y = Var1, fill = value)) + geom_tile() + scale_fill_gradient2() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
Sparcc_plot2
...
sparCCpcors <- diag(0.5, nrow = dim(sparcc.chi1$Cor)[1], ncol = dim(sparcc.chi1$Cor)[1]) #gives a 38 x 38 matrix
sparCCpcors[upper.tri(sparCCpcors, diag=FALSE)] <- cors
sparCCpcors <- sparCCpcors + t(sparCCpcors) 

sparCCpval <- diag(0.5, nrow = dim(sparcc.chi1$Cor)[1], ncol = dim(sparcc.chi1$Cor)[1]) #38 x 38 matrix
sparCCpval [upper.tri(sparCCpval, diag=FALSE)] <- pvals 
sparCCpval <- sparCCpval + t(sparCCpval)
...
#now need to process and account for false discovery rates 
sparccCor_processed <- sparCCpcors %>%  reshape2::melt() %>% na.omit() %>% rename(cor = value)
sparccP_processed <- sparCCpval %>% reshape2::melt() %>% na.omit() %>% rename(p = value)
...
#If we turn var1 and var2 from factors into characters, ggplot will put them in alphabetical order based on our code: 

SparccP$Var1 <- as.character(SparccP$Var1)
SparccP$Var2 <- as.character(SparccP$Var2)

SparccP_plot2h_v2 <- SparccP %>% ggplot(aes(x = Var2, y = Var1, fill = cor)) + geom_tile() +
  scale_fill_gradientn(
  colors = c("#800000FF", "white", "#4D004B"), limits = c(-1, 1)) + scale_x_discrete(limits=rev, position = "top") + scale_y_discrete(limits=rev, position = "right") + theme(
  axis.text.x = element_markdown(angle = 45, hjust = 0, face = "italic"),
  axis.text.y = element_markdown(face = "italic"),
...
ggsave("/Users/swetapatel/OneDrive - Duke University/Fogarty coding/Fig4SparCC_June24_FULL_setseedV3.pdf", plot = SparccP_plot2h_v2, dpi = 300, width = 10, height = 7)

#comparing our second SparccP dataframe to a screenshot of the first, they are identical. To compare our full SparccP to the half one we are going to generate to re-do our co-occurrence plot, will save the full SparccP as a csv
write.csv(SparccP, "/Users/swetapatel/OneDrive - Duke University/Fogarty coding/SparCC_June2024_setseed_fullV2.csv")

#NEXT: re-run our ORIGINAL code to generate half the heatmap, make sure the values correlate with the full heatmap since we used the same set.seed, and then export this to Cytoscape to regenerate our co-occurrence diagram (after clearing our environment to reset from the last full SparCC run)
...
#now save as a csv:
write.csv(SparccP, "/Users/swetapatel/OneDrive - Duke University/Fogarty coding/SparCC_June2024_setseed_halfV2.csv")

#comparing csv files: the cor and p values are identical, but the fdr values are different, so we will REDO everything and add a set.seed above the fdr code

#the below code is our troubleshooting to get the species in alphabetical order; can disregard as we resolved it by converting Var1 and Var2 from factors --> character variables

png(file="/Users/swetapatel/OneDrive - Duke University/Fogarty coding/sparcc_fullV1.png",
    width = 12, height = 9, units = 'in', res = 600)
SparccP_plot2g
dev.off()

#we have a full heat map! But now we need to organize alphabetically
class(SparccP$Var1) #factors
class(SparccP$Var2)

SparccP %>% ggplot(aes(x = Var2, y = Var1, fill = cor)) + geom_tile()  +  scale_fill_gradientn(
  colors = c("#800000FF", "white", "#4D004B"), limits = c(-1, 1)) + ylim(levels(SparccP$Var1)) + xlim(levels(SparccP$Var1)) + theme(
  axis.text.x = element_markdown(angle = 45, hjust = 0, face = "italic"),
  axis.text.y = element_markdown(face = "italic"),
  legend.title = element_text(size = 9),
  legend.text.align = 1,
...
#no combination of xlim/ylim or scale_x_discrete/scale_y_discrete is working to re-order the variables; we will save our new SparccP output that gives us a full heat map as a csv file, then open that csv file and attempt an alphabetical heat map that way:
write.csv(SparccP, "/Users/swetapatel/OneDrive - Duke University/Fogarty coding/SparCCfull_June2024.csv")
SparccPcsv <- read.csv("/Users/swetapatel/OneDrive - Duke University/Fogarty coding/SparCCfull_June2024.csv")

class(SparccPcsv$Var1)

fdrThresh2 <- 0.1 # fdr threshold
sparccOkP2 <- SparccPcsv%>% filter(fdr < fdrThresh2) 

SparccP_plot2h <- SparccPcsv %>% ggplot(aes(x = Var2, y = Var1, fill = cor)) + geom_tile() +
  scale_fill_gradientn(
  colors = c("#800000FF", "white", "#4D004B"), limits = c(-1, 1)) + scale_x_discrete(limits=rev, position = "top") + scale_y_discrete(limits=rev, position = "right") + theme(
  axis.text.x = element_markdown(angle = 45, hjust = 0, face = "italic"),
  axis.text.y = element_markdown(face = "italic"),
...
ggsave("/Users/swetapatel/OneDrive - Duke University/Fogarty coding/Fig4SparCC_June24_FULL.pdf", plot = SparccP_plot2h, dpi = 300, width = 10, height = 7)

#Now we will re-generate our network diagram in Cytoscape using the newest csv file for continuity.
...
clr_cd4
###UPDATE JUNE 2025 with reviewer requested annotation
p3 <- clr_cd4 + annotate("text", x = 26, y = -30, label = "p = 0.013", size = 10/.pt)

p3

#does our p value text placement line up for both figures? will generate a test figure to check and then delete
test <- plot_grid(p1, p3, labels = "AUTO", nrow = 2, ncol = 1, align = "h", axis = "b")
test
ggsave("/Users/swetapatel/OneDrive - Duke University/Fogarty coding/test_annotatedpcoa.pdf", plot = test, dpi = 300, height = 9, width = 8)
# dev.off()
...
adonis(BC.dist ~ cd4nL, data = chi_cd4, permutations = 1000)  #P=0.01, R = 0.042
  #CD4 + age (per 6/9 MK discussion to account for age in all PERMANOVAs)
adonis2(BC.dist ~ cd4nL + age, data = chi_cd4, permutations = 1000) #P=0.013, R = 0.042 for CD4; age P = 0.20
...
subject_plot_cd4_2 <- ggplot(test2[order(test2$Species, decreasing = TRUE),], aes(x=cd4cat, y=subject_abundances, fill=Species)) + geom_bar(stat="identity", position="fill") + theme_classic() + theme(legend.position="right", legend.text=element_markdown(), axis.title.y = element_text(size=12, margin=margin(0,20,0,0)), axis.text.y = element_text(size=10), axis.title.x = element_blank(), axis.text.x = element_text(size=10, angle = 45, vjust = 1, hjust = 1, colour = "black")) + scale_fill_manual(values = chisp2, labels = c("*Corynebacterium accolens*", "*Corynebacterium propinquum*", "*Corynebacterium pseudodiphtheriticum*", "*Dolosigranulum pigrum*", "*Haemophilus influenzae*", "*Moraxella catarrhalis*", "*Moraxella lincolnii*", "*Moraxella nonliquefaciens*",  "*Staphylococcus aureus*", "*Streptococcus pneumoniae*", "Other")) + ylab("Mean relative abundance")
...
# ggsave("/Users/swetapatel/OneDrive - Duke University/Fogarty coding/fig2_annotated_JID062025.pdf", plot = fig2, dpi = 300, height = 9, width = 11)

# ggsave(file="/Users/swetapatel/OneDrive - Duke University/Fogarty coding/fig2_JID.tiff", width = 11, height = 9, units = "in", device='tiff', dpi=300)
# fig2
# dev.off()
#still giving artifact, which is weird, at both dpi=300 and dpi=700

#Saving as TIFF for JID 2025:
tiff("/Users/swetapatel/OneDrive - Duke University/Fogarty coding/fig2_JID.tiff", width = 11, height = 9, units = "in", res = 600, compression = "lzw")
fig2
dev.off()
...
#Significant difference by HIV status among children (P = 0.002997, R2 = 0.032) and age (P=0.03, R2 = 0.015)
#check with age first given issues with order of variables in adonis; not signif different
...
### Colonization prevalence of S. pneumo and S. aureus

- using melted_df and code from 10-6-22 Kaiju file
...
kruskal.test(sa$Abundance, sa$hiv)  #P=0.049; borderline to no difference in relative abundances by HIV status

#what about differences in presence/absence of S. aureus?
sa$present <- NA
...
summary(table(sa$present, sa$hiv)) #there is a significant difference by HIV status (p=0.027)
...
summary(table(spneum$present, spneum$hiv))  #Trend towards higher %HEU kids with S.pneumo col (96% vs 82% in HIV+ and 86% in HUU) but not signif; p=0.094


```

### Final Lancet Microbe comments
- April 2025 update for JID previously reviewed papers pathway submission
- reviewer concerned about our sequencing depth and level of host contamination, so we can gather some data from the kneaddata_counts csv file MK generated when running pipeline on the initial sequencing run 

```{r}
kneaddata <- read.csv("/Users/swetapatel/OneDrive - Duke University/Fogarty coding/Sequencing/HIV_NGS_kneaddata_counts.csv")
summary(kneaddata$raw.pair1)

#create variable that is trimmed reads - final reads; is this a rough approximation of the #reads removed by bowtie2? Will check with MK
kneaddata$hum <- (kneaddata$trimmed.pair1 - kneaddata$final.pair1)
kneaddata$ratio <- (kneaddata$final.pair1/kneaddata$hum)
summary(kneaddata$ratio)
kneaddata$ratio2 <- (kneaddata$final.pair1/kneaddata$trimmed.pair1)
summary(kneaddata$ratio2)
```

### Final JID comments

- Our paper was provisionally accepted to JID (woo!) but we need to submit our figures as .eps or .tiff files, not PDF (which unfortunately is not stated in the image guidance for authors document: https://static.primary.prod.gcms.the-infra.com/static/site/journals/document/images-author-guidance.pdf?node=1bf05d0b2fbd9c529a23&version=490455:30c2211aa70bba63a5ee)

- per ChatGPT, will attempt to use magick package in R to convert Figure 2, figure 3, and figure 5. It was straightforward to regenerate fig 1 (enrollment diagram) and fig 4 (maaslin output, done in the maaslin barplot section above), but since we need to set.seed, etc for the code that generated figs 2/3/5 I don't want to run into reproducibility or coding problems.

```{r}
library(magick)
library(pdftools)
fig2 <- image_read_pdf("/Users/swetapatel/OneDrive - Duke University/Fogarty coding/fig2_annotated_JID062025.pdf", density = 300)

image_write(fig2, path = "/Users/swetapatel/OneDrive - Duke University/Fogarty coding/fig2_JID.tiff", format = "tiff")

#THERE IS ARTIFACT even when we lower our dpi from 600 to 300; so we will recreate Fig2 in R and be very mindful to set.seed, etc

#what about fig 3? SUCCESS!
clust <- image_read_pdf("/Users/swetapatel/OneDrive - Duke University/Fogarty coding/Fig3NEW(clust)_V3.pdf", density = 600)

image_write(clust, path = "/Users/swetapatel/OneDrive - Duke University/Fogarty coding/fig3_JID.tiff", format = "tiff")

#fig 5
fig5 <- image_read_pdf("/Users/swetapatel/OneDrive - Duke University/Fogarty coding/Fig4SparCC_June24_FULL_setseedV3 annotated.pdf", density = 600)

image_write(fig5, path = "/Users/swetapatel/OneDrive - Duke University/Fogarty coding/fig5_JID.tiff", format = "tiff")
