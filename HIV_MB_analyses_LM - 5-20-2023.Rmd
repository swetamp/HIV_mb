...
# otu <- otu[-c(11, 25, 37, 38, 40),]
# 
# # import our new otu table back into chi.filter, then re-create the dataframe and go from there
# #import updated otutable into pseq object (convert to matrix first) and take a look
# TT <- as.matrix(otu)
# otu2 <- otu_table(TT, taxa_are_rows = TRUE)
# class(otu2)
# otu_table(chi.filter) <- otu2
# #can we test it to see if it worked? 
# test <- as.data.frame(otu_table(chi.filter)) #does not work; kicks out the renamed rows
# remove(TT, test, otu2)
...
# #what happens here if we modify our rownames and colnames?
# lapply(sparcc.chi1$Cor, `colnames<-`, paste0("Haemophilus haemolyticus", 3))
# lapply(sparcc.chi1$Cor, "dimnames", paste0("Haemophilus haemolyticus", 3))
# 
# row.names(otu)[3] <- "Haemophilus haemolyticus"
# row.names(otu)[10] <- "Corynebacterium pseudodiphtheriticum"
# row.names(otu)[11] <- "Corynebacterium accolens"
...
###Will try to rename species in SparccP and see if that helps with figure creation
#create second dataframe given amt time required to bootstrap P values (so if doesn't work, we still have our original SparccP)
#make Var1 and Var2 character variables, then rename using gsubfn
# SparccP2 <- SparccP
# SparccP2$Var1 <- as.character(SparccP2$Var1)
# SparccP2$Var2 <- as.character(SparccP2$Var2)
# SparccP2$Var1 <- gsub("Corynebacterium sp. KPL1995", "Corynebacterium pseudodiphtheriticum", SparccP2$Var1)
# SparccP2$Var1 <- gsub("Corynebacterium sp. KPL1986", "Corynebacterium accolens", SparccP2$Var1)
# SparccP2$Var1 <- gsub("Haemophilus sp. C1", "Haemophilus haemolyticus", SparccP2$Var1)
# 
# SparccP2$Var2 <- gsub("Corynebacterium sp. KPL1995", "Corynebacterium pseudodiphtheriticum", SparccP2$Var2)
# SparccP2$Var2 <- gsub("Corynebacterium sp. KPL1986", "Corynebacterium accolens", SparccP2$Var2)
# SparccP2$Var2 <- gsub("Haemophilus sp. C1", "Haemophilus haemolyticus", SparccP2$Var2)
...
# toString(test$Var1)
# toString(test$Var2)
# write.csv(test, "/Users/swetapatel/OneDrive - Duke University/Fogarty coding/SparCCredo_cortax.csv")

# #per below link will try to save as a text file instead
# #link: https://groups.google.com/g/cytoscape-helpdesk/c/4XHpiWWAjRE
# #save as .txt file and try again
# write.table(SparccP, "/Users/swetapatel/OneDrive - Duke University/Fogarty coding/SparCCredo_cortax.txt")
# write.table(SparccP, "/Users/swetapatel/OneDrive - Duke University/Fogarty coding/SparCCredo_cortax.tsv", quote=FALSE, sep = '\t')
